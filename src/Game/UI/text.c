...
int txt_LoadFont( const char* fileName, int pixelHeight )
{
	...
	fonts[newFont].packageID = img_SplitAlphaBitmap( bmpBuffer, bmpWidth, bmpHeight, fontPackRange.num_chars, ST_ALPHA_ONLY, GL_NEAREST, mins, maxes, retIDs );
...
static void bindFontTask( void* data )
{
	...
	fonts[newFont].packageID = img_SplitAlphaBitmap( fontData->bmpBuffer, fontData->bmpWidth, fontData->bmpHeight,
		fontData->packRange.num_chars, ST_ALPHA_ONLY, GL_NEAREST, mins, maxes, retIDs );
	if( fonts[newFont].packageID < 0 ) {
...
static int loadSDFFont( const char* fileName )
{
	...
	fonts[fontID].packageID = img_SplitAlphaBitmap( pngImg.data, pngImg.width, pngImg.height, fontPackRange.num_chars, ST_SIMPLE_SDF, GL_LINEAR, mins, maxes, retIDs );
...
#define CHECK_POINTER( p, s ) \
	...
	fonts[newFont].packageID = img_SplitAlphaBitmap( baseImage, WIDTH, HEIGHT, fontPackRange.num_chars, ST_SIMPLE_SDF, GL_LINEAR, mins, maxes, retIDs );
	if( fonts[newFont].packageID < 0 ) {

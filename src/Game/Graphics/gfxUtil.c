...
int gfxUtil_CreateTextureFromLoadedImage( GLenum texFormat, LoadedImage* image, Texture* outTexture, GLint filter )
{
...
int gfxUtil_LoadTexture( const char* fileName, Texture* outTexture )
{
	...
	if( gfxUtil_CreateTextureFromLoadedImage( GL_RGBA, &image, outTexture, GL_NEAREST ) < 0 ) {
		returnCode = -1;
		goto clean_up;
	}
...
//  Returns >= 0 on success, < 0 on failure.
int gfxUtil_CreateTextureFromAlphaBitmap( uint8_t* data, int width, int height, Texture* outTexture, GLint filter )
{
	assert( data != NULL );
	assert( width > 0 );
	assert( height > 0 );
	...
	texFormat = GL_RED;
#endif
	if( gfxUtil_CreateTextureFromLoadedImage( texFormat, &image, outTexture, filter ) < 0 ) {
		returnCode = -1;
		goto clean_up;
	}

...
#define DEFAULT_FRAG_SHADER \
	...
	"in vec4 vCol;\n" \
	"uniform sampler2D textureUnit0;\n" \
	"uniform float floatVal0; // used for greyscale, 0 is none, 1 is full\n" \
	"out vec4 outCol;\n" \
	"void main( void )\n" \
	"{\n" \
    "   vec4 baseCol = texture2D(textureUnit0, vTex) * vCol;\n" \
	"   float grey = ( baseCol.r * 0.3 ) + ( baseCol.g * 0.59 ) + ( baseCol.b * 0.11 );\n" \
    "   vec4 greyCol = vec4( grey, grey, grey, baseCol.a );\n" \
	"	outCol = mix( baseCol, greyCol, floatVal0 );\n" \
	"   //outCol = vCol;\n" \
	"	if( outCol.w <= 0.0f ) {\n" \
	"		discard;\n" \
	"	}\n" \

...
#define MAX_RENDER_INSTRUCTIONS ( 1024 * 8 )
typedef struct {
	Vector2 pos;
	Vector2 scaledSize;
	Color color;
...
int img_CreateFromLoadedImage( LoadedImage* loadedImg, ShaderType shaderType, const char* id )
{
	...
	Texture texture;
	if( gfxUtil_CreateTextureFromLoadedImage( GL_RGBA, loadedImg, &texture, GL_NEAREST ) < 0 ) {
...
static void bindImageJob( void* data )
{
	...
	Texture texture;
	if( gfxUtil_CreateTextureFromLoadedImage( GL_RGBA, &( loadData->loadedImage ), &texture, GL_NEAREST ) < 0 ) {
...
Takes in a one channel bitmap and some rectangles. It's assume the length of the mins, maxes, and retIDs equal scount.
 Returns package ID used to clean up later, returns -1 if there's a problem.
*/
int img_SplitAlphaBitmap( uint8_t* data, int width, int height, int count, ShaderType shaderType, GLint filter, Vector2* mins, Vector2* maxes, int* retIDs )
{
	int currPackageID = findUnusedPackage( );

	Texture texture;
	if( gfxUtil_CreateTextureFromAlphaBitmap( data, width, height, &texture, filter ) < 0 ) {
...
int img_Draw_v( int imgID, uint32_t camFlags, Vector2 startPos, Vector2 endPos, float startFloatVal0, float endFloatVal0, int8_t depth )
{
	DRAW_INSTRUCTION_START;
